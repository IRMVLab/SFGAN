...
def train():
    ...
    with tf.Graph().as_default():       #默认图
        with tf.device('/gpu:'+str(GPU_INDEX)):     #制定GPU
            ...
            loss = CC_loss + 10*smooth_loss + 10*Chamfer_loss + 3*Curvature_loss
...
def scene_flow_EPE_np(pred, labels):
    ...
    # error = np.linalg.norm(pred - labels, axis=-1)
    num = pred.shape[1]

    gtflow_len = np.sqrt(np.sum(labels*labels, 2) + 1e-20) # B,N
    # gtflow_len = np.linalg.norm(labels, axis=-1)
...
def eval_one_epoch_kitti(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        for save_idx in range(start_idx, end_idx):
            real_idx = save_idx - start_idx
            fname = os.path.join(LOG_DIR, str(save_idx).zfill(6)+'.npz')
            pred_epe = pred_val[real_idx, :NUM_POINT, :3].reshape(1, -1, 3)
            label_epe = batch_label[real_idx, :NUM_POINT, :3].reshape(1, -1, 3)
            # print(pred_epe.shape)
            acc_mask = scene_flow_mask_np(pred_epe, label_epe)
            epe, ac1, ac2, oli = scene_flow_EPE_np(pred_epe, label_epe)
            log_string(str(save_idx))
            log_string('epe: %f' %(epe))
            np.savez_compressed(fname, pc1 = batch_data[real_idx, :NUM_POINT, :3], pc2 = batch_data[real_idx, NUM_POINT:, :3], 
                                pc2_pred = batch_data[real_idx, :NUM_POINT, :3] + pred_val[real_idx, :NUM_POINT, :3], 
                                pc2_acc_mask = acc_mask,
                                pc2_label = batch_data[real_idx, :NUM_POINT, :3] + batch_label[real_idx, :NUM_POINT, :3])
