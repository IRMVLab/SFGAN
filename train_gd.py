...
def train():
    ...
    with tf.Graph().as_default():       #默认图
        with tf.device('/gpu:'+str(GPU_INDEX)):     #制定GPU
            ...
            loss = CC_loss + W_S*smooth_loss + Chamfer_loss + Curvature_loss
...
def train_one_epoch_d(sess, ops, train_writer):
    ...
    # train_idxs = np.arange(0, 19640)
    train_idxs = np.arange(0, 19640)
    np.random.shuffle(train_idxs)
    #num_batches = len(TRAIN_DATASET) // BATCH_SIZE
    num_batches = 19640 // BATCH_SIZE
    ...
    for batch_idx in range(num_batches):
        ...
        end_idx = min(19640, (batch_idx+1) * BATCH_SIZE)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    log_string('g eval mean loss: %f' % (g_loss_sum / float(3840/BATCH_SIZE)))
    log_string('d eval mean loss: %f' % (d_loss_sum / float(3840/BATCH_SIZE)))
    log_string('eval mean loss: %f' % (loss_sum_l2 / float(3840/BATCH_SIZE)))


    epe3d = sum_epe3d / float(3840 // BATCH_SIZE)
    acc3d_1 = sum_acc3d_1 / float(3840 // BATCH_SIZE)
    acc3d_2 = sum_acc3d_2 / float(3840 // BATCH_SIZE)
    outlier = sum_outlier / float(3840 // BATCH_SIZE)
    ...
    outfile.write('g eval mean loss: %f' % (g_loss_sum / float(3840/BATCH_SIZE)) + '\n')
    outfile.write('d eval mean loss: %f' % (d_loss_sum / float(3840/BATCH_SIZE)) + '\n')
    outfile.write('eval mean loss: %f' % (loss_sum_l2 / float(3840/BATCH_SIZE)) + '\n')
    outfile.close()
