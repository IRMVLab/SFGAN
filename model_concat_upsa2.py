...
def Discriminator(point_cloud, flow, is_training, bn_decay=None):
    ...
    with tf.variable_scope('dis', reuse=tf.AUTO_REUSE) as scope_1:
        ...
        with tf.variable_scope('sa3', reuse=tf.AUTO_REUSE) as scope_4:
            _, l2_points_f1_fake = flow_embedding_module(l2_xyz_f1, l2_xyz_fake, l2_points_f1, l2_points_fake, radius=10, nsample=64, mlp=[128,128,128], is_training=is_training, bn_decay=bn_decay, bn=True, scope='flow_embedding', pooling='max', knn=True, corr_func='concat')
            scope_4.reuse_variables()
            _, l2_points_f1_real = flow_embedding_module(l2_xyz_f1, l2_xyz_real, l2_points_f1, l2_points_real, radius=10, nsample=64, mlp=[128,128,128], is_training=is_training, bn_decay=bn_decay, bn=True, scope='flow_embedding', pooling='max', knn=True, corr_func='concat')
