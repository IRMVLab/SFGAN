...
parser.add_argument('--weight_supervised', type=float, default=50.0)
...
W_G = FLAGS.weight_g
W_SUPER = FLAGS.weight_supervised
# W_S = FLAGS.weight_smooth
...
TRAIN_DATASET_KITTI = kitti_dataset.SceneflowDataset(DATA_KITTI, npoints=NUM_POINT, train=True)
TEST_DATASET_KITTI = kitti_dataset.SceneflowDataset(DATA_KITTI, npoints=NUM_POINT, train=False)
...
def train():
    ...
    with tf.Graph().as_default():       #默认图
        with tf.device('/gpu:'+str(GPU_INDEX)):     #制定GPU
            ...
            supervised_loss = MODEL.get_loss(pred, labels_pl)

            #grads = optimizer.compute_gradients(loss)

            
            # g_loss = MODEL.GLoss(d_model_real, d_logits_real, d_model_fake, d_logits_fake, is_training_pl_g)
            # g_loss_total = W_G*g_loss + WEIGHT_LOSS*loss + W_SUPER *supervised_loss
            g_loss_total =  WEIGHT_LOSS*loss + W_SUPER *supervised_loss
        ...
        for epoch in range(MAX_EPOCH):
            ...
            epe3d, acc3d_1, acc3d_2, outlier = eval_one_epoch_kitti(sess, ops, test_writer)
...
def train_one_epoch_d(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_data, batch_label = get_eval_batch(TRAIN_DATASET_KITTI, train_idxs, start_idx, end_idx)
        ...
        summary, step, _, g_loss_val, d_loss_val, smooth_loss, cf_loss, CC_loss, cv_loss, loss, supervised_loss, g_loss_total, pred_val, d_logits_real_val, d_logits_fake_val = sess.run(
            [ops['merged'], ops['step'],
            ops['d_train_op'],
            ops['g_loss'], ops['d_loss'], 
            ops['smooth_loss'], ops['chamfer_loss'], ops['CC_loss'], ops['curvature_loss'], ops['loss'], ops['supervised_loss'], ops['g_loss_total'], 
            ops['pred'], 
        ...
        if (batch_idx+1)%10 == 0:
            ...
            log_string('supervised mean loss: %f' % (supervised_loss_sum / 10))
...
def train_one_epoch_g(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        feed_dict = {ops['pointclouds_pl']: batch_data,
                    ...
                    ops['is_training_pl_g']: is_training_g,}
                    # ops['is_training_pl_d']: is_training_d}
        summary, step, _, smooth_loss, cf_loss, CC_loss, cv_loss, loss, supervised_loss, g_loss_total, pred_val= sess.run(
            [ops['merged'], ops['step'],
            ops['g_train_op'],
            ops['smooth_loss'], ops['chamfer_loss'], ops['CC_loss'], ops['curvature_loss'], ops['loss'], ops['supervised_loss'], ops['g_loss_total'], 
            ops['pred'] 
