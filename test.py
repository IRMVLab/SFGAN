...
# TRAIN_DATASET = flying_things_dataset.SceneflowDataset(DATA, npoints=NUM_POINT, train=0)
# TEST_DATASET = flying_things_dataset.SceneflowDataset(DATA, npoints=NUM_POINT, train=1)
TEST_DATASET = kitti_dataset.SceneflowDataset(DATA, npoints=NUM_POINT)
...
def train():
    ...
    with tf.Graph().as_default():       #默认图
        with tf.device('/gpu:'+str(GPU_INDEX)):     #制定GPU
            ...
            # g_loss_total = g_loss + WEIGHT_LOSS*loss
        ...
        # init = tf.variables_initializer(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='const|op|dis'))
        init = tf.variables_initializer(var_list=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='const'))
        sess.run(init)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    test_idxs = np.arange(0, 150)

    # Test on all data: last batch might be smaller than BATCH_SIZE
    num_batches = (150+BATCH_SIZE-1) // BATCH_SIZE
    ...
    for batch_idx in range(num_batches):
        ...
        end_idx = min(150, (batch_idx+1) * BATCH_SIZE)
        cur_batch_size = end_idx-start_idx
        cur_batch_data, cur_batch_label = get_eval_batch(TEST_DATASET, test_idxs, start_idx, end_idx)
        ...
        summary, step, pred_val= sess.run(
            [ops['merged'], ops['step'],
            #ops['loss'],
    ...
    # log_string('g eval mean loss: %f' % (g_loss_sum / float(150/BATCH_SIZE)))
    # log_string('d eval mean loss: %f' % (d_loss_sum / float(150/BATCH_SIZE)))
    log_string('eval mean loss: %f' % (loss_sum_l2 / float(150/BATCH_SIZE)))


    epe3d = sum_epe3d / float(150 // BATCH_SIZE)
    acc3d_1 = sum_acc3d_1 / float(150 // BATCH_SIZE)
    acc3d_2 = sum_acc3d_2 / float(150 // BATCH_SIZE)
    outlier = sum_outlier / float(150 // BATCH_SIZE)
    ...
    # # outfile.write('g eval mean loss: %f' % (g_loss_sum / float(150/BATCH_SIZE)) + '\n')
    # # outfile.write('d eval mean loss: %f' % (d_loss_sum / float(150/BATCH_SIZE)) + '\n')
    # outfile.write('eval mean loss: %f' % (loss_sum_l2 / float(150/BATCH_SIZE)) + '\n')
    # outfile.close()
